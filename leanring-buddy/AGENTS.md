...
- Keep Studio tab content in dedicated scene files where practical, and compose shared cards, rows, button treatments, palette, and shell elements from the `CompanionStudio*` chrome files instead of duplicating local styling inside the root Studio view.
- Keep outer window/header/backdrop behavior in `CompanionStudioChrome`, palette tokens in `CompanionStudioPalette`, shared cards/rows in `CompanionStudioSharedComponents`, button treatments in `CompanionStudioButtonModifiers`, and account/access visuals in `CompanionStudioAccessChrome`.
- Keep Companion Studio hero/journey, personalization, connection/computer-use, and access areas in dedicated `CompanionStudioCompanion*` section card files. `CompanionStudioCompanionScene` should stay a composition root.
...
- Keep screen-aware primary narrative copy in `CompanionPanelPrimaryContentCard`, setup/access secondary cards in `CompanionPanelAccessCards`, and tutorial-specific secondary cards in `CompanionPanelTutorialCards`; `CompanionPanelView` should compose those modules and own only panel state/actions.
- `CompanionPanelPermissionRows` owns permission-row construction from a snapshot plus action set. Keep permission repair row branching there instead of rebuilding rows in `CompanionPanelView`.
- Keep reusable cursor overlay indicators in `OverlayIndicators`, AV/WebKit player bridges in `OverlayVideoPlayers`, bubble/media surfaces in `BlueCursorSurfaces`, navigation mode and flight math in `BlueCursorNavigation`, SwiftUI cursor composition in `BlueCursorView`, and AppKit window lifecycle in `OverlayWindowManager`.
...
- `ClickyAssistantRuntime` owns assistant backend adapter construction, turn execution, response repair, and response processing. Keep provider setup there rather than reintroducing backend wiring directly into `CompanionManager`.
- The normal assistant response path should use one structured response contract with spoken text plus ordered point targets. `ClickyAssistantPresentationPolicy` owns transcript pointing heuristics, point-target resolution, and managed pointing narration. Do not depend on backend-specific inline tag syntax in the main companion flow; legacy `[POINT:...]` tags are onboarding-demo-only.
- `ClickyTutorialStateStore` persists the current tutorial draft and session progress locally. Do not treat that local snapshot as backend-backed or cross-device tutorial history without an explicit product decision.
- For OpenClaw-backed Clicky turns, use the plugin-exposed `clicky_present` tool to choose the presentation mode, then have the agent emit the exact structured JSON envelope returned by that tool as the final assistant message.
