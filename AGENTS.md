...
- **Studio Scene Structure**: Keep Studio tab content in dedicated scene files where practical, and compose shared cards, rows, button treatments, palette, and shell elements from the `CompanionStudio*` chrome files instead of duplicating local styling inside the root Studio view.
- **Studio Chrome Structure**: Keep outer window/header/backdrop behavior in `CompanionStudioChrome`, palette tokens in `CompanionStudioPalette`, shared cards/rows in `CompanionStudioSharedComponents`, button treatments in `CompanionStudioButtonModifiers`, and account/access visuals in `CompanionStudioAccessChrome`.
- **Companion Studio Sections**: Keep Companion Studio hero/journey, personalization, connection/computer-use, and access areas in dedicated `CompanionStudioCompanion*` section card files. `CompanionStudioCompanionScene` should stay a composition root.
- **Companion Panel Structure**: Keep reusable menu-bar panel screen models, permission rows, button styles, shell styling, transitions, and status chips in `CompanionPanelChrome` instead of redefining them inline in the panel body.
- **Companion Panel Flow**: `CompanionPanelFlowState` owns pure menu-bar panel screen selection from onboarding, auth, permission, paywall, and tutorial import inputs. Keep new screen-selection branches there rather than scattering switches through `CompanionPanelView`.
- **Companion Panel Cards**: Keep screen-aware primary narrative copy in `CompanionPanelPrimaryContentCard`, setup/access secondary cards in `CompanionPanelAccessCards`, and tutorial-specific secondary cards in `CompanionPanelTutorialCards`; `CompanionPanelView` should compose those modules and own only panel state/actions.
- **Companion Panel Permission Rows**: `CompanionPanelPermissionRows` owns permission-row construction from a snapshot plus action set. Keep permission repair row branching there instead of rebuilding rows in `CompanionPanelView`.
- **Overlay Structure**: Keep reusable cursor overlay indicators in `OverlayIndicators`, AV/WebKit player bridges in `OverlayVideoPlayers`, bubble/media surfaces in `BlueCursorSurfaces`, navigation mode and flight math in `BlueCursorNavigation`, SwiftUI cursor composition in `BlueCursorView`, and AppKit window lifecycle in `OverlayWindowManager`.
...
- **Assistant Runtime Graph**: `ClickyAssistantRuntime` assembles assistant backend adapters, turn execution, response repair, and response processing. Keep provider construction there rather than reintroducing backend setup directly into `CompanionManager`.
- **Computer Use Direction**: BackgroundComputerUse owns the desktop-control API through its MCP server and model instruction resource. Clicky should host or locate the MCP helper, surface readiness and permission/review state, and pass the provider-neutral MCP contract to model runtimes. Production builds bundle `BackgroundComputerUseMCP` plus Clicky's MCP policy proxy into `Clicky.app/Contents/Helpers`; sibling checkouts and `swift run` are development fallbacks only. Do not duplicate route schemas, action clients, or tool wrappers inside Clicky.
...
- **OpenClaw Presentation Path**: For OpenClaw-backed Clicky turns, use the plugin-exposed `clicky_present` tool to choose the presentation mode, then have the agent emit the exact structured JSON envelope returned by that tool as the final assistant message.
